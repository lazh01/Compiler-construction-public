...
%token <stringconst> tIDENTIFIER '+' '-' '*' '/' AND OR EQ LEQ GEQ NEQ INC DEC '<' '>'
%token <boolconst> tBOOL
%token <nullconst>  tNULL
%token <error> ERROR

%token RETURN WHILE ALLOCATE WRITE OFLENGTH IF ELSE THEN DO FOR TYPEY ARRAYOF RECORDOF VARI FUNC INT BOOL END
%type <exp> expression
...
statement : RETURN expression ';'
            ...
            | WHILE expression DO statement
                  {$$ = makeSTATEwhile($2,$4);}
            | FOR '(' decl_list ')' INC '[' term ',' term ']' statement
                  {$$ = makeSTATEforinc($3,$7,$9,$11);}
            | FOR '(' decl_list ')' DEC '[' term ',' term ']' statement
                  {$$ = makeSTATEfordec($3,$7,$9,$11);}
            | '{' statement_list '}'
