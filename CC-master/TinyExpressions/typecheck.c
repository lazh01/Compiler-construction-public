...
void checkSTATE(STATE *s, COLLECTION *c){
     switch(s->kind){
          ...
          case stateListK:
               ...
               break;
          case incK:
               checkVAR(s->val.incS, c);
               break;
          case decK:
               checkVAR(s->val.decS, c);
               break;
          case forincK:
               checkDECL_LIST(s->val.forincS.first, c);
               checkTERM(s->val.forincS.second, c);
               checkTERM(s->val.forincS.third, c);
               checkSTATE(s->val.forincS.fourth, c);
               break;
          case fordecK:
               checkDECL_LIST(s->val.fordecS.first, c);
               checkTERM(s->val.fordecS.second, c);
               checkTERM(s->val.fordecS.third, c);
               checkSTATE(s->val.fordecS.fourth, c);
               break;
     }
}
