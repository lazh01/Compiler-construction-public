...
bool statementListChecker(STATE_LIST *sl){
     ...
     if(slLeft->kind == ifElseK){
          ...
          if(ifChecker(slLeft) == true){
               if(slRight != NULL){
               printf(CYN "Note: Return statement on line %d does not allow the end of code to be reached.\n" RESET, slLeft->lineno);
               }
               return true;
          }
     }
...
void weedFunction(FUNCTION *f){
    ...
    if(strcmp(headID,tailID) != 0){
        printf(RED "Error: Id of function:" RESET);
        printf(BLU " \"%s\" ", headID);
        printf(RED "does not match end id. line %d\n" RESET, (f->val.function.left)->lineno);
        printf("exiting...\n");
        exit(0);
    }
     weedHEAD(f->val.function.left);
     if(statementListChecker((f->val.function.middle)->val.body.right) == false){
          printf(MAG "Warning: Function: " RESET);
          printf(BLU "\"%s\" ", headID);
          printf(MAG "might not end with a return statement. line %d\n" RESET, (f->val.function.left)->lineno);
     }
